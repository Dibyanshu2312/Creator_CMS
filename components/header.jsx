...
const path = usePathname();
  return (
    <header className='fixed top-6 left-1/2 transform -translate-x-1/2 z-50 w-full max-w-3xl px-4'> 
        ...
        {path === "/" && (<div className="hidden lg:flex space-x-6  flex-1 justify-center">
            <Link href="#features" 
            className="text-white font-medium transition-all duration-300 hover:text-purple-300 cursor-pointer">
              Features
            </Link>
            <Link href="#testimonials" className="text-white font-medium transition-all duration-300 hover:text-purple-300 cursor-pointer">
            Testimonials
            </Link>
            </div>)}
...
<div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
        ...
        {isLoading && (
          <div className='fixed bottom-0 left-0 w-full z-40 flex justify-center'>
            <BarLoader width={"95%"} color="#D884FE" ></BarLoader>
          </div>
        )}
